...
public class ItemStockpileItem extends BlockItem {
	...
	private void tryMultiPlace(BlockPlaceContext ctx) {
		...
		if (!ItemStockpileBlock.isVault(placedOnState))
			return;
		ItemStockpileBlockEntity tankAt = ConnectivityHandler.partAt(DesiresBlockEntityTypes.ITEM_STOCKPILE.get(), world, placedOnPos);
		if (tankAt == null)
			return;
		ItemStockpileBlockEntity controllerBE = tankAt.getControllerBE();
		...
		int tanksToPlace = 0;
		Axis vaultBlockAxis = ItemStockpileBlock.getVaultBlockAxis(placedOnState);
