...
public class FluidKegItem extends BlockItem {

    public FluidKegItem(Block p_i48527_1_, Properties p_i48527_2_) {
    ...
    private void tryMultiPlace(BlockPlaceContext ctx) {
        ...
        if (!FluidKegBlock.isTank(placedOnState))
            return;
        FluidKegBlockEntity tankAt = ConnectivityHandler.partAt(DesiresBlockEntityTypes.FLUID_KEG.get(), world, placedOnPos);
        if (tankAt == null)
            return;
        FluidKegBlockEntity controllerBE = tankAt.getControllerBE();
        ...
        int tanksToPlace = 0;
        Axis vaultBlockAxis = FluidKegBlock.getKegAxis(placedOnState);
        ...
        for (int xOffset = 0; xOffset < width; xOffset++) {
            for (int zOffset = 0; zOffset < width; zOffset++) {
                ...
                if (FluidKegBlock.isTank(blockState))
                    continue;
